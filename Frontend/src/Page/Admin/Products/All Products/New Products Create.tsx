...
const NewProductsCreate = ({ onBack }: AddAccountProps) => {
  ...
  const [form, setForm] = useState({
    ...
    weight: "" as number | "",
    description: "",
    originType: "Local", // "Local" or "International"
    country: "",
    countryFlag: "",
    images: [] as string[],
    availability: "On Hand", // "On Hand" or "Order Online"
    deliveryTime: "1-3 Days Delivery", // Auto-updates based on availability
    soldOut: false,
    quantity: 0,
    minQty: 0,
    maxQty: 0,
  ...
  const handleCountryChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setForm(prev => ({ 
      ...
      // Automatically fetch high-quality flag image URL using standard ISO codes
  ...
  const handleVariantImageUpload = async (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      try {
        ...
        // Process multiple uploads concurrently for the variant
        const uploadPromises = filesArray.map(async (file) => {
          const compressed = await compressImage(file);
          return await uploadToImgBB(compressed);
        });
      ...
      <Dialog open={confirmDialogOpen} onClose={() => setConfirmDialogOpen(false)} PaperProps={{ sx: { borderRadius: "24px", p: 1 } }}>
        ...
        <DialogContent>
            <Typography sx={{ fontFamily: primaryFont, color: "#475569" }}>
                You are about to add <b>{form.name}</b> to the catalog. It will be immediately available in the catalog.
            </Typography>
        </DialogContent>
